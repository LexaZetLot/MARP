...
import net.comand_marp.marp.item.ModArmorMaterials;
...
public class ModArmorItem extends ArmorItem {
    ...
    private boolean hasCorrectArmorOn(ArmorMaterial material, PlayerEntity player) {
        ...
        boolean bootsCorrect = (equippableComponentBoots != null && equippableComponentBoots.assetId()
                .map(asset -> asset.equals(material.assetId()))
                .orElse(false));

        boolean leggingsCorrect = (equippableComponentLeggings != null && equippableComponentLeggings.assetId()
                .map(asset -> asset.equals(material.assetId()))
                .orElse(false));

        boolean breastplateCorrect = (equippableComponentBreastplate != null && equippableComponentBreastplate.assetId()
                .map(asset -> asset.equals(material.assetId()))
                .orElse(false));

        boolean helmetCorrect = (equippableComponentHelmet != null && equippableComponentHelmet.assetId()
                .map(asset -> asset.equals(material.assetId()))
                .orElse(false));

        return bootsCorrect && leggingsCorrect && breastplateCorrect && helmetCorrect;
    }
}